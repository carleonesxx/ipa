...
@MainActor final class SessionStore: ObservableObject {
    ...
    func signIn(email: String, password: String) async { loading = true; defer { loading = false }; do { let r = try await api.login(email: email, password: password); user = r.user; isAuthenticated = true; error = nil } catch { error = error.localizedDescription } }
    func signUp(name: String, email: String, password: String) async { loading = true; defer { loading = false }; do { let r = try await api.register(name: name, email: email, password: password); user = r.user; isAuthenticated = true; error = nil } catch { error = error.localizedDescription } }
